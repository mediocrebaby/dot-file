...
describe("mission store", () => {
	it("moves mission records and their global index outside the project in traceless mode", () => {
		const test = fixture();
		try {
			const location = resolveMissionStoreLocation({
				...
				config: { directory: path.join(test.projectRoot, ".pi-subagents", "ignored"), globalIndexDir: path.join(test.projectRoot, ".pi-subagents", "ignored-index") },
			});
			const dataDir = getPiSubagentDataDir(test.agentDir);
			assert.equal(location.missionDir, path.join(dataDir, "missions", getProjectStorageKey(test.projectRoot)));
			assert.equal(location.globalIndexDir, path.join(dataDir, "missions", "index"));
			assert.equal(location.missionDir.startsWith(test.projectRoot), false);
		} finally {
