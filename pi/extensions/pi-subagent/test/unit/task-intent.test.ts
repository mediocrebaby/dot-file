...
describe("classifyTaskMutationIntent", () => {
	...
	it("uses task wording rather than predefined role names", () => {
		for (const agent of ["researcher", "reviewer", "oracle", "advisor", "custom-role"]) {
			assert.equal(classifyTaskMutationIntent(agent, "Research this and patch the bug").kind, "implementation");
			assert.equal(classifyTaskMutationIntent(agent, "Review this; regardless of findings, apply changes directly").kind, "implementation");
			assert.equal(classifyTaskMutationIntent(agent, "Review only; return findings and do not edit files").kind, "read-only");
		}
	});
