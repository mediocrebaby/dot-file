...
describe("acceptance gates", () => {
	...
	it("uses explicit read-only metadata instead of agent-name heuristics", () => {
		const resolved = resolveEffectiveAcceptance({
			agentName: "oracle",
			acceptanceRole: "read-only",
			task: "Review prep findings and recommend the next steps.",
			mode: "single",
			async: true,
		});

		assert.equal(resolved.level, "attested");
		assert.deepEqual(resolved.inferredReason, ["declared read-only acceptance role"]);
	...
	it("agent contract v1 keeps explicit acceptance report-optional and verify-only", async () => {
		...
		const verified = resolveEffectiveAcceptance({
			agentName: "worker",
			task: "Implement the fix",
			explicit: { level: "verified", verify: [{ id: "ok", command: `${JSON.stringify(process.execPath)} -e "process.exit(0)"` }] },
			agentContract: { version: 1 },
		});
