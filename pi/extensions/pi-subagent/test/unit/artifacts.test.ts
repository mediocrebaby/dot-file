...
describe("project-local artifact paths", () => {
	...
	it("forces artifacts off and temp storage in project traceless mode", () => {
		const resolved = resolveArtifactConfig({
			config: { artifactDir: "project", projectTracelessMode: true },
			artifacts: true,
			persisted: { enabled: true, dir: "project", includeInput: true, includeOutput: true, includeJsonl: true, includeTranscript: true, includeMetadata: true, cleanupDays: 30 },
		});
		assert.equal(resolved.enabled, false);
		assert.equal(resolved.dir, "temp");
	});
});
