...
describe("subagents.defaultExtensions", () => {
	...
	it("persists per-agent extension overrides", () => {
		const override = buildAgentOverrideConfig(
			{
	...
	it("uses project settings over user settings while respecting discovery scope", () => {
		...
		writeProjectAgent("scout");
		const userScoutPath = path.join(tempHome, ".agents", "scout.md");
		fs.mkdirSync(path.dirname(userScoutPath), { recursive: true });
		fs.writeFileSync(userScoutPath, `---\nname: scout\ndescription: User scout\n---\n\nScout.\n`, "utf-8");
