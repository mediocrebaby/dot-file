...
import { getPiSubagentDataDir, getProjectStorageKey } from "../../src/shared/extension-paths.ts";
...
describe("Herdr inspector", () => {
	...
	it("stores project pane bindings outside the project in traceless mode", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-herdr-project-pane-traceless-"));
		...
		try {
			...
			const opened = await handleHerdrProjectPaneAction("project.open", { cwd: projectRoot }, {
				cwd: root,
				client,
				projectTracelessMode: true,
			});
			assert.equal(opened.isError, undefined, text(opened));
			assert.equal(fs.existsSync(path.join(projectRoot, ".pi-subagents")), false);
			const canonicalProjectRoot = fs.realpathSync(projectRoot);
			const bindingPath = path.join(getPiSubagentDataDir(agentDir), "project-panes", getProjectStorageKey(canonicalProjectRoot), "herdr.json");
			assert.equal(fs.existsSync(bindingPath), true);
			assert.equal(readHerdrProjectPaneBinding(canonicalProjectRoot, true)?.paneId, "w1:p11");
		} finally {
