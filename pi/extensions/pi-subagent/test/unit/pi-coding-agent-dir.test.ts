...
describe("PI_CODING_AGENT_DIR runtime paths", () => {
	...
	it("resolves the agent dir dynamically and loads extension config from it", () => {
		...
		process.env.PI_CODING_AGENT_DIR = agentDir;
		const configPath = path.join(agentDir, "extensions", "pi-subagent", "config.json");
		writeFile(configPath, JSON.stringify({ asyncByDefault: true, maxSubagentDepth: 3, artifactDir: "session", projectTracelessMode: true }));

		const config = loadConfig();
		assert.equal(config.asyncByDefault, true);
		assert.equal(config.maxSubagentDepth, 3);
		assert.equal(config.artifactDir, "session");
		assert.equal(config.projectTracelessMode, true);
	});
...
Use env agent.
`);
		writeFile(path.join(agentDir, "agents", "worker.md"), `---
name: worker
...
`);
		writeFile(settingsPath, JSON.stringify({
			subagents: {
				agentOverrides: {
					worker: { model: "openai/env-rooted" },
				},
			},
		}, null, 2));
		...
		assert.equal(worker?.model, "openai/env-rooted");
...
`);
	...
	it("uses the configured agent dir for subagent bridge instruction files", () => {
		const instructionPath = path.join(agentDir, "extensions", "pi-subagent", "bridge.md");
