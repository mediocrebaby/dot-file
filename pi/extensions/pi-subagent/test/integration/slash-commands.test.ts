...
describe("slash command custom message delivery", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	beforeEach(() => {
		clearSlashSnapshots?.();
		projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pi-slash-run-"));
		const agentDir = path.join(projectRoot, ".pi", "agents");
		fs.mkdirSync(agentDir, { recursive: true });
		fs.writeFileSync(path.join(agentDir, "scout.md"), "---\nname: scout\ndescription: Slash command test agent\n---\n\nInspect the requested target.\n", "utf-8");
	});
	...
	it("/run accepts an agent without a task", async () => {
		...
		const ctx = createCommandContext({ cwd: projectRoot, sessionManager });
