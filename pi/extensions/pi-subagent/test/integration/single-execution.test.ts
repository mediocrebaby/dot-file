...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("routes registered structured text delegation through the concurrent executor", async () => {
		...
		const projectAgentDir = path.join(tempDir, ".pi", "agents");
		fs.mkdirSync(projectAgentDir, { recursive: true });
		fs.writeFileSync(path.join(projectAgentDir, "worker.md"), "---\nname: worker\ndescription: Executes a delegated test node\n---\n\nPerform the requested work.\n", "utf-8");
		fs.writeFileSync(path.join(projectAgentDir, "reviewer.md"), "---\nname: reviewer\ndescription: Executes a second delegated test node\n---\n\nReview the requested work.\n", "utf-8");
	...
	it("silently overrides project artifact and chain settings in traceless mode", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ output: "traceless chain result" });
		const executor = makeExecutor([makeAgent("echo")], { artifactDir: "project", projectTracelessMode: true });
		const requestedChainDir = path.join(tempDir, ".pi-subagents", "explicit-chain-dir");

		const result = await executor.execute(
			"traceless-chain",
			{ chain: [{ agent: "echo", task: "Use {chain_dir}" }], artifacts: true, chainDir: requestedChainDir },
			new AbortController().signal,
			undefined,
			makeMinimalCtx(tempDir),
		);
