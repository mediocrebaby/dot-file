...
### Added
- Added global `projectTracelessMode` in `~/.pi/agent/extensions/pi-subagent/config.json`; it disables debug artifacts, moves chain scratch to OS temp, and stores mission/Herdr project state under the extension data directory without touching existing project `.pi-subagents` content.
...
### Changed
- Removed all bundled subagent definitions and name-based role defaults. Executable agents now come exclusively from package, user, or project Markdown definitions.
- Removed builtin-only management/model/profile surfaces and bundled prompt recipes that depended on predefined role names.
- Changed the extension installation and writable config root from `extensions/subagent` to `extensions/pi-subagent` without compatibility migration.
