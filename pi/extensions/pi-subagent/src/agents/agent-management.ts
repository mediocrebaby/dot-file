...
function allAgents(d: { package: AgentConfig[]; user: AgentConfig[]; project: AgentConfig[] }): AgentConfig[] {
...
function findAgents(name: string, cwd: string, scope: AgentScope = "both"): AgentConfig[] {
	...
	const scoped = mergeAgentsForScope(scope, d.user, d.project, d.package);
...
export function handleUpdate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (params.agent) {
		...
		if (target.source !== "user" && target.source !== "project") return result(`Cannot update ${target.source} agent '${target.name}'. Create an editable user or project agent with the same runtime name to override it.`, true);
	...
	if ("content" in targetOrError) return targetOrError;
	const target = targetOrError;
	if (target.source !== "user" && target.source !== "project") return result(`Cannot update ${target.source} chain '${target.name}'. Create an editable user or project chain with the same runtime name to override it.`, true);
