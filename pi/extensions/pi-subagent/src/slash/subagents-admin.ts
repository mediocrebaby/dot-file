...
async function chooseOverrideScope(ctx: ExtensionContext, agent: AgentConfig): Promise<"user" | "project" | undefined> {
	...
	const choice = await ctx.ui.select(`Save settings override for ${agent.name}`, ["user", "project"]);
...
function persistSettingsField(
	...
	if (value === undefined || value === base[field]) {
		return {
			filePath: removeAgentOverrideFields(ctx.cwd, agent.name, scope, [field]).path,
			overridden: false,
		};
	}
	return {
		filePath: mergeAgentOverride(ctx.cwd, agent.name, scope, { [field]: value }),
		overridden: true,
	};
}
