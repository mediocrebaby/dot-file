...
import { getPiSubagentConfigPath } from "../shared/extension-paths.ts";
...
export function getConfigPath(): string {
	return getPiSubagentConfigPath();
}
...
function readConfigForUpdate(configPath = getConfigPath()): ExtensionConfig {
	...
	if (config.projectTracelessMode !== undefined && typeof config.projectTracelessMode !== "boolean") {
		throw new Error("config.projectTracelessMode must be a boolean");
	}
