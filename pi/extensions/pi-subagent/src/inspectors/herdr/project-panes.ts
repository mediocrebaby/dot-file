...
import { getPiSubagentDataDir, getProjectStorageKey } from "../../shared/extension-paths.ts";
...
function projectPaneDir(projectRoot: string, projectTracelessMode: boolean): string {
	if (projectTracelessMode) {
		return path.join(getPiSubagentDataDir(), "project-panes", getProjectStorageKey(projectRoot));
	}
	return path.join(projectRoot, ".pi-subagents", "project-panes");
}

function bindingPath(projectRoot: string, projectTracelessMode = false): string {
	return path.join(projectPaneDir(projectRoot, projectTracelessMode), "herdr.json");
}
...
export function readHerdrProjectPaneBinding(projectRoot: string, projectTracelessMode = false): HerdrProjectPaneBinding | undefined {
	try { return parseBinding(JSON.parse(fs.readFileSync(bindingPath(projectRoot, projectTracelessMode), "utf-8"))); } catch { return undefined; }
}
...
export async function handleHerdrProjectPaneAction(action: HerdrProjectPaneAction, params: ProjectPaneParams, deps: ProjectPaneDeps): Promise<AgentToolResult<Details>> {
	...
	const projectTracelessMode = deps.projectTracelessMode === true;
	const projectBindingPath = bindingPath(projectRoot, projectTracelessMode);
	const existing = readHerdrProjectPaneBinding(projectRoot, projectTracelessMode);
	...
	if (action === "project.close") {
		...
		fs.rmSync(projectBindingPath, { force: true });
	...
	writeAtomicJson(projectBindingPath, binding);
