...
function missionForRun(asyncDir: string, cwd: string, config: MissionStoreConfig | undefined, runId: string, projectTracelessMode: boolean): { id: string; path: string } | undefined {
	try {
		...
		const location = resolveMissionStoreLocation({
			projectRoot: cwd,
			...(config ? { config } : {}),
			...(projectTracelessMode ? { projectTracelessMode: true } : {}),
		});
...
export async function handleHerdrInspectorAction(action: HerdrInspectorAction, params: InspectorParams, deps: InspectorDeps): Promise<AgentToolResult<Details>> {
	...
	const mission = missionForRun(target.asyncDir, deps.cwd, deps.missions, target.runId, deps.projectTracelessMode === true);
