...
export class ScheduledRunManager {
	...
	async handleToolCall(params: SubagentParamsLike, ctx: ExtensionContext): Promise<AgentToolResult<Details>> {
		...
		try {
			if (!scheduledRunsEnabled(this.deps.config)) {
				return textResult("Scheduled subagent runs are disabled by { \"scheduledRuns\": { \"enabled\": false } } in ~/.pi/agent/extensions/pi-subagent/config.json. Remove that opt-out or set enabled to true, then reload Pi. Schedule only explicit delayed runs the user asked for.", true);
			}
