...
function inferLevel(input: {
	...
	const declaredRoleWriteTask = input.acceptanceRole !== undefined
		&& intent.kind !== "read-only"
		&& (/\b(?:create|add|replace)\b(?!\s+(?:(?:a|an|the)\s+)?(?:report|summary|findings?|analysis|proposal|plan)\b)/.test(task)
			|| (!/\b(?:do not|don't|must not)\s+patch\b/.test(task)
				&& /\bpatch\s+(?:(?:\.{0,2}[\\/])?(?:[\w.-]+[\\/])+[\w.-]+|[\w.-]+\.[a-z0-9]+\b|(?:the\s+)?parser\b)/.test(task)));
	const taskMayWrite = readOnlyTask ? false : taskMayMutate(input.task ?? "") || intent.kind === "implementation" || declaredRoleWriteTask;
...
export async function evaluateAcceptance(input: {
	...
	const requiresStructuralChecks = LEVEL_RANK[acceptance.level] >= LEVEL_RANK.checked
		|| acceptance.criteria.length > 0
		|| acceptance.evidence.length > 0;
	if (parsed.report && requiresStructuralChecks) {
		...
		if (LEVEL_RANK[acceptance.level] >= LEVEL_RANK.checked && !ledger.runtimeChecks.some((check) => check.status === "failed")) {
