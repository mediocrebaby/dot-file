...
const GENERAL_IMPLEMENTATION_PATTERNS = [
	...
	/\bdo those fixes\b/i,
	/\b(?:update|add|remove|replace|delete|create)\s+(?:the\s+)?(?:file|files|code|source|implementation|functionality|features?|references?|test|tests|component|function|module|class|method|logic|import|imports|readme|docs?|changelog|package\.json|config|manifest|extension|prompt|command)\b/i,
	/\b(?:remove|delete)\s+(?:the\s+)?(?:[\w.-]+\s+){0,2}(?:functionality|features?|references?)\b/i,
	/\b(?:replace|update)\s+(?:the\s+)?(?:registered|native|existing|current|legacy)\s+(?:command|component|function|module|extension|implementation)\b/i,
	/\bcreate\s+(?:the\s+)?[\w.-]+\.[a-z0-9]+\b/i,
];
...
export function classifyTaskMutationIntent(_agent: string, task: string): TaskMutationIntent {
	...
	if (READ_ONLY_TASK_PATTERNS.some((pattern) => pattern.test(taskText))) return { kind: "read-only" };
