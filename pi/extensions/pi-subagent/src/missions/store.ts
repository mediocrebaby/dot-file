...
export function resolveMissionStoreLocation(input: {
	...
	const tracelessMissionRoot = path.join(getPiSubagentDataDir(agentDir), "missions");
	const missionDir = input.projectTracelessMode === true
		? path.join(tracelessMissionRoot, getProjectStorageKey(projectRoot))
		: input.config?.directory
			? expandConfiguredPath(input.config.directory, projectRoot)
			: path.join(projectRoot, ".pi-subagents", "missions");
	const globalIndexDir = input.projectTracelessMode === true
		? path.join(tracelessMissionRoot, "index")
		: input.config?.globalIndexDir
