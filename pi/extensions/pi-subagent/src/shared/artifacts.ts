...
export function resolveArtifactConfig(input: {
	config: Pick<ExtensionConfig, "artifactDir" | "projectTracelessMode">;
	artifacts?: boolean;
	persisted?: ArtifactConfig;
}): ArtifactConfig {
	if (input.config.projectTracelessMode === true) {
		return { ...(input.persisted ?? DEFAULT_ARTIFACT_CONFIG), enabled: false, dir: "temp" };
	}
	return input.persisted ?? {
		...DEFAULT_ARTIFACT_CONFIG,
		enabled: input.artifacts !== false,
		dir: input.config.artifactDir ?? DEFAULT_ARTIFACT_CONFIG.dir,
	};
}
