...
The extension ships no predefined agents. After installing, first ask Pi which Markdown agents are available:

```text
Show me the available subagents.
```

Pi can discover agents supplied by installed packages, `~/.pi/agent/agents/`, `.agents/`, and `.pi/agents/`. If the list is empty, create a project agent such as `.pi/agents/reviewer.md` using the example under [Common workflows](#common-workflows), then ask:

```text
Use the configured reviewer agent to review this diff.
```

No additional extension config or slash command is required.
...
Substitute names returned by `subagent({ action: "list" })` for the placeholders below:

```text
Ask <advice-agent> for a second opinion on my current plan. Challenge assumptions and tell me what I might be missing.
```

```text
Use <investigation-agent> to inspect this hard bug and propose the best next move before we edit anything.
```

```text
Run three fresh read-only agents on this diff: one for correctness, one for tests, and one for unnecessary complexity.
```

```text
Have <implementation-agent> implement this approved plan. Afterward, run fresh read-only reviewers, summarize their feedback, and apply the fixes that make sense.
```

```text
Run a review loop on this change until the selected review agents stop finding fixes worth doing, with a max of 3 rounds.
```

```text
Use <discovery-agent> to understand the auth flow, then have <planning-agent> turn that into an implementation plan.
```

Those are ordinary Pi requests. Pi decides whether to call `subagent` and how to express composed work with `workflowScript`, but it must select only discovered agents. Agent names do not carry hidden behavior.

## Common workflows

| Want | Ask naturally |
|------|---------------|
| Get a second opinion | “Ask a discovered advice or review agent to challenge this plan.” |
| Solve a hard problem | “Use a discovered investigation agent to inspect this bug before we edit.” |
| Review a diff | “Use the configured read-only review agent to review this diff.” |
| Run parallel reviews | “Run fresh read-only agents for correctness, tests, and cleanup.” |
| Implement then review | “Use the configured implementation agent, then run independent review agents.” |
| Review until clean | “Run a review loop with the selected implementation and review agents, capped at 3 rounds.” |
| Execute a plan carefully | “Use the configured implementation agent for this plan, then review and apply justified fixes.” |
| Discover before planning | “Use the configured discovery agent before the planning agent.” |
...
The extension does not ship any predefined agents. Every executable capability comes from a discovered Markdown agent definition. Define any role names used in your own prompts, or substitute names from `subagent({ action: "list" })`.

A minimal project agent at `.pi/agents/reviewer.md` looks like this:
...
Inspect the requested files or diff and return evidence-backed findings. Do not edit unless explicitly asked.
```

## Changing an agent's model

Markdown agents inherit your current Pi default model unless their frontmatter or settings select another model. Set `subagents.defaultModel` for a shared default, or use an `agentOverrides` entry for a specific discovered agent.
...
For a persistent override, edit settings. This example pins a configured `reviewer` agent and adds a backup model for provider failures:
...
Use `~/.pi/agent/settings.json` for a user override or the project config settings file (`.pi/settings.json` in standard Pi) for a project override. `subagents.defaultModel` applies to package, user, and project agents that do not set `model` in frontmatter. Per-run model overrides and `agentOverrides.<name>.model` still win, and explicit agent frontmatter remains authoritative. Matching Markdown agents receive override fields that their frontmatter leaves unset, so a shared definition can keep the persona while local settings choose the model.
...
Use `/subagents` or `subagent({ action: "get", agent: "<name>" })` to inspect the effective model and metadata for a discovered agent.
...
`/subagents-fleet` opens the live fleet inspector with current-session foreground work, recent async children, structured Markdown/tool transcripts, and completed output/session paths. Use `←`/`→` to focus the Agents or Detail pane, then `↑`/`↓` or `j`/`k` to select a child or scroll its conversation. Use `o` in the Detail pane or `x`/`Ctrl+O` globally to toggle tool details, `r` to refresh, and `Esc` to close. For a selected live async child, `s` sends an acknowledged steer message and `D` stops its top-level async run after confirmation. `Ctrl+Alt+F` opens the same inspector even while a foreground turn is active and slash input is queued. Without a TUI, `/subagents-fleet` retains the textual `subagent({ action: "status", view: "fleet" })` fallback, and mutations use explicit commands: run `/subagents-stop` and pick from the selector, or use `/subagents-stop <run-id>` / `subagent({ action: "stop", id: "..." })` when you already know the id. Use `/subagents-detach [run-id]` only for an active foreground single-subagent run you want to leave running without terminating; the eventual result remains available through status/wait. To inspect one background child in text, use `subagent({ action: "status", id: "...", view: "transcript" })`; add `index` for a specific child in a parallel or chain run.

FleetView replaces the legacy above-editor async widget by default. Successful background completion messages remain visually quiet, but they still wake the originating parent session so it can continue without an explicit wait; failed and paused completions remain visible. Parallel runs show every active child independently. Chains with parallel groups keep their grouped shape in progress and results, so failed or paused agents stay visible next to completed ones. When a child is explicitly allowed to fan out with `tools: subagent`, its nested runs appear under that parent child in the main status tree instead of being hidden inside the child process.
...
Use orchestration as parent-agent guidance, not as a runtime workflow mode. For implementation work, a useful loop is:

```text
clarify → planning-capable agent → implementation-capable agent → fresh read-only review agents → implementation-capable agent
```

Select every agent from `subagent({ action: "list" })`. Context behavior comes only from the launch request or the selected agent's Markdown frontmatter; names such as `planner`, `worker`, or `reviewer` have no implicit defaults. Save repeatable orchestration as a Markdown/JSON chain or a prompt workflow that explicitly names agents which exist in your environment.
...
```text
Run this implementation with the configured implementation agent in the background. If it gets blocked or needs a product decision, have it ask me through intercom.
```

```text
Ask the configured review agent to review this plan. If it sees a decision I need to make, have it ask me instead of assuming.
```
...
Native permissions are opt-in and apply only to Pi child runtimes. With no rules configured, every tool call passes through unchanged. Configure explicit non-bash rules globally in `~/.pi/agent/extensions/pi-subagent/config.json`:
...
Settings can supply fields that a discovered Markdown agent does not own in frontmatter:
...
Supported override fields are `description`, `model`, `fallbackModels`, `thinking`, `systemPromptMode`, `inheritProjectContext`, `inheritSkills`, `defaultContext`, `acceptanceRole`, `disabled`, `skills`, `tools`, `extensions`, and `subagentOnlyExtensions`. Frontmatter-owned fields remain authoritative; project overrides beat user overrides.

Set `subagents.defaultModel` to give all discovered agents without an explicit model their own default model, separate from the parent session model. Per-agent model overrides and agent frontmatter still win.
...
No role name receives hidden defaults. Every Markdown agent defaults to `systemPromptMode: replace`, `inheritProjectContext: false`, and `inheritSkills: false`; opt into broader context explicitly in frontmatter.
...
- **Discovery-first delegation**: list available package, user, and project Markdown agents before choosing single, parallel, chain, or async execution
- **Prompting guidance**: write compact task contracts from each selected agent's declared description, tools, context, and acceptance metadata
- **Implementation and review recipes**: keep one writer per cwd/worktree and use fresh read-only agents for independent review
...
If you are writing an agent that orchestrates subagents, the bundled skill helps it discover and use configured agents without guessing hidden roles. Human users can follow the same discovery-first patterns in this README.
...
These examples assume `scout`, `planner`, and `reviewer` are Markdown agents already present in the discovery list; substitute your configured names.
...
{ action: "update", agent: "code-analysis.scout", config: { acceptanceRole: false } } // clear the declared role and fall back to task-based inference
...
| `context` | `fresh \| fork` | per-agent default or `fresh` | Explicit `fresh` or `fork` overrides every child. When omitted, each agent uses its Markdown-defined `defaultContext`; `fork` creates real branched sessions from the parent leaf. |
...
`pi-subagents` reads optional JSON config from `~/.pi/agent/extensions/pi-subagent/config.json`. The old `extensions/subagent` path is not read or migrated.
...
Controls the under-editor widget for active background runs. It defaults to `false`; set it to `true` to show the widget, including alongside FleetView. Background execution, completion notifications, lifecycle events, and FleetView remain available while this widget is hidden.
...
- `instructionFile`: optional Markdown template replacing the default bridge instructions. `{orchestratorTarget}` is interpolated. Relative paths resolve from `~/.pi/agent/extensions/pi-subagent/`.
...
### `projectTracelessMode`

```json
{ "projectTracelessMode": true }
```

Defaults to `false`. When enabled, future pi-subagents behavior never creates or writes the current project's `.pi-subagents` directory. The mode silently takes precedence over `artifactDir`, per-run `artifacts`, explicit project-local `chainDir`, and mission storage overrides:

- debug artifacts are disabled;
- child Pi sessions remain enabled outside the project, and Fleet uses runtime-owned session transcripts when debug transcripts are unavailable; after an extension restart, disk-only inspection intentionally reopens only sessions under configured or derived trusted roots;
- chain scratch files use the user-scoped OS temp directory;
- mission records move to `~/.pi/agent/extensions/pi-subagent/data/missions/<project-hash>/`;
- the mission global index moves to `~/.pi/agent/extensions/pi-subagent/data/missions/index/`;
- Herdr project-pane bindings move to `~/.pi/agent/extensions/pi-subagent/data/project-panes/<project-hash>/`.

The mode affects only future writes. It does not inspect, migrate, or delete an existing project `.pi-subagents` directory.
...
Controls smart batching of async-completion notifications. When several background subagents finish within a short window, their successful completions are held briefly and delivered as a single visually quiet grouped completion instead of separate completions. The grouped notification still wakes the originating parent session. A hard `maxWaitMs` cap (measured from the first completion in a group) guarantees nothing is held indefinitely, and late-finishing siblings that arrive within `stragglerWindowMs` of a group emit join a shorter straggler group governed by `stragglerDebounceMs` and `stragglerMaxWaitMs`.
...
Async completions belong only to the originating session. The result watcher emits `subagent:async-complete`, and the extension consumes that event to record completion state. Successful sibling completions are held briefly and delivered as a visually quiet grouped completion when they finish within a short window (see `completionBatch`); the notification still wakes the parent session. Failed and paused completions remain visible and fire immediately.
...
`pi-subagents` works standalone through natural language, the `subagent` tool, and slash commands. It also includes a native prompt-workflow adapter for reusable user or project prompt templates, so you do not need `pi-prompt-template-model` for this workflow path.
