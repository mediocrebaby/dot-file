...
const TOOL_FOLD_PATCH_FLAG = Symbol.for("pi-claude-style-tools:patched-tool-fold-level");
const SPACER_FOLD_PATCH_FLAG = Symbol.for("pi-claude-style-tools:patched-spacer-fold-level");
...
interface SettingsFile {
	...
	/** Default Ctrl+O detail level for every new, resumed, switched, forked, or reloaded session. */
	toolFoldDefault?: ToolFoldLevelName;
...
let FOLD_WORKING_FG = "\x1b[38;5;208m";
let FOLD_ACTION_THINK = "\x1b[35m";
let FOLD_ACTION_BASH = "\x1b[33m";
let FOLD_ACTION_READ = "\x1b[34m";
let FOLD_ACTION_SEARCH = "\x1b[36m";
let FOLD_ACTION_EDIT = "\x1b[32m";
let FOLD_ACTION_WRITE = "\x1b[38;5;37m";
let FOLD_ACTION_PATCH = "\x1b[38;5;70m";
let FOLD_ACTION_ASK = "\x1b[38;5;171m";
let FOLD_ACTION_AGENT = "\x1b[38;5;209m";
let FOLD_ACTION_TASK = "\x1b[38;5;69m";
let FOLD_ACTION_SKILL = "\x1b[38;5;214m";
let FOLD_ACTION_MCP = "\x1b[38;5;99m";
let FOLD_ACTION_LIST = "\x1b[38;5;45m";
...
/** 压缩摘要行最多展示的动作类别数，其余类别合并为 `+N`。 */
const MAX_FOLD_ACTION_SEGMENTS = 4;
...
// ===========================================================================
// Ctrl+O 三档显示层级：逐条摘要 / 完整输出 / 整组压缩
// ===========================================================================

let toolFoldLevel: ToolFoldLevel = 0;
/**
 * Pi 的展开布尔值只有 ctrl+o 会让它翻转，用它识别按键次数：每次翻转推进一档。
 * 组内往子工具传播展开状态时临时加深 foldPropagationDepth，不会被误当成按键。
 */
let lastPiExpandedValue: boolean | undefined;
let foldPropagationDepth = 0;
let foldUiContext: any;

/** Pi 的布尔值永远等于「是否处于完整输出档」，按键后由 syncPiExpandedToFoldLevel 纠偏。 */
function notePiToolsExpanded(expanded: boolean): void {
	if (foldPropagationDepth > 0) return;
	if (lastPiExpandedValue === undefined) {
		lastPiExpandedValue = expanded;
		return;
	}
	if (lastPiExpandedValue === expanded) return;
	lastPiExpandedValue = expanded;
	toolFoldLevel = nextToolFoldLevel(toolFoldLevel);
	queueMicrotask(syncPiExpandedToFoldLevel);
}

function syncPiExpandedToFoldLevel(): void {
	const ui = foldUiContext;
	if (!ui || typeof ui.getToolsExpanded !== "function" || typeof ui.setToolsExpanded !== "function") return;
	const desired = isToolFoldExpanded(toolFoldLevel);
	if (ui.getToolsExpanded() !== desired) {
		foldPropagationDepth++;
		try {
			ui.setToolsExpanded(desired);
		}
		finally {
			foldPropagationDepth--;
		}
	}
	lastPiExpandedValue = desired;
}

function configuredToolFoldLevel(): ToolFoldLevel {
	return toolFoldLevelFromName(readSettings().toolFoldDefault);
}

/** 会话重建后读取持久化默认档，并与 Pi 当前的展开布尔值对齐。 */
function resetToolFoldLevel(ui: any): void {
	toolFoldLevel = configuredToolFoldLevel();
	foldUiContext = ui;
	const expanded = typeof ui?.getToolsExpanded === "function" ? ui.getToolsExpanded() : undefined;
	lastPiExpandedValue = typeof expanded === "boolean" ? expanded : undefined;
	queueMicrotask(syncPiExpandedToFoldLevel);
}

function foldParticipantTools(value: any): any[] | undefined {
	if (isToolGroupComponent(value)) return value.getTools();
	// Compressed mode owns the whole work run, including edit/write/patch tools.
	// NON_GROUPABLE_TOOL_NAMES still controls the steps layout only.
	if (value instanceof ToolExecutionComponent) return [value];
	return undefined;
}

function isFoldRunParticipant(value: unknown): boolean {
	return foldParticipantTools(value) !== undefined || isFoldThoughtParticipant(value);
}

function isThinkingRowComponent(inner: any): boolean {
	if (!inner) return false;
	if (inner instanceof HiddenThinkingSummary || inner?.constructor?.name === "HiddenThinkingSummary") return true;
	return isHiddenThinkingPlaceholderText(inner);
}

/**
 * 整条消息只渲染出思考行的助手组件。消息本身可能同时带工具调用，
 * 那种情况下工具行是独立子组件，这里只判断渲染内容是不是只有思考行。
 */
function assistantMessageHasThinking(value: unknown): boolean {
	if (!value || (value as any)?.constructor?.name !== "AssistantMessageComponent") return false;
...
function isFoldThoughtParticipant(value: unknown): boolean {
	if (!assistantMessageHasThinking(value)) return false;
	const children = (value as any).contentContainer?.children;
	if (!Array.isArray(children) || children.length === 0) return false;
	let sawThinkingRow = false;
	for (const child of children) {
		if (isSpacerComponent(child)) continue;
		const inner = (child as any)?.child ?? child;
		if (!isThinkingRowComponent(inner)) return false;
		sawThinkingRow = true;
	}
	return sawThinkingRow;
}
...
/**
 * 收集组件所在的批次：连续的“工具组 / 工具行 / 思考行”算一批，
 * 中间的空行不打断批次，其它组件（正文、用户消息、总结）是批次边界。
 */
function getFoldRun(component: any): FoldRun | undefined {
	if (!isFoldRunParticipant(component)) return undefined;
	const children = component?.[COMPONENT_PARENT]?.children;
	if (!Array.isArray(children)) return undefined;
	const index = children.indexOf(component);
	if (index === -1) return undefined;
	const { start, end } = foldRunBounds(
		children.length,
		index,
		(i) => isFoldRunParticipant(children[i]),
		(i) => isIgnorableToolSeparator(children[i]),
	);
	const run: FoldRun = { tools: [], thoughts: [] };
	for (let i = start; i <= end; i++) {
		const child = children[i];
		if (isFoldRunParticipant(child) && run.first === undefined) run.first = child;
		const tools = foldParticipantTools(child);
		if (tools) {
			run.tools.push(...tools);
		} else if (isFoldThoughtParticipant(child)) {
			run.thoughts.push(child);
		}
	}
	// Final assistant messages commonly contain both a finished thinking block
	// and visible prose. They terminate the work run, yet their thinking still
	// belongs to the work immediately before the answer.
	for (let i = end + 1; i < children.length; i++) {
		const child = children[i];
		if (isIgnorableToolSeparator(child)) continue;
		if (assistantMessageHasThinking(child) && !isFoldThoughtParticipant(child)) run.thoughts.push(child);
		break;
	}
	return run.tools.length > 0 || run.thoughts.length > 0 ? run : undefined;
}

function foldThoughtClaimedByPreviousRun(component: any): boolean {
	const children = component?.[COMPONENT_PARENT]?.children;
	...
	for (let i = index - 1; i >= 0; i--) {
		...
		if (!isFoldRunParticipant(sibling)) return false;
		return getFoldRun(sibling)?.thoughts.includes(component) === true;
	}
	return false;
}

/** 结果渲染器写在行尾的快捷键提示，压缩行里必须剥掉。 */
function foldHintFragments(): string[] {
	const expandKey = keyText("app.tools.expand").trim() || "ctrl+o";
	return [
		`${expandKey} to toggle`,
		`${expandKey} to expand`,
		`${expandKey} to collapse`,
		`${expandKey} for steps`,
		"ctrl+shift+o more detail",
		"ctrl+shift+o less detail",
	];
}

/** 读取工具结果渲染器留下的单行摘要，例如 Done (37 lines)。 */
function foldToolResultText(tool: any, width: number): string {
	const component = (tool as any)?.resultRendererComponent;
	let text = typeof component?.value === "string" ? component.value : "";
	if (!text && typeof component?.render === "function") {
		text = component.render(width).join("\n");
	}
	if (!text) return "";
	return extractFoldResultSummary(stripAnsi(text), foldHintFragments());
}

function themeFoldMuted(text: string): string {
	return `${WORKED_LINE_FG}${text}${TRANSPARENT_RESET}`;
}

function foldActionColor(label: string): string {
	if (label === "Think") return FOLD_ACTION_THINK;
	if (label === "Bash") return FOLD_ACTION_BASH;
	if (label === "Read") return FOLD_ACTION_READ;
	if (label === "Search") return FOLD_ACTION_SEARCH;
	if (label === "List") return FOLD_ACTION_LIST;
	if (label === "Edit") return FOLD_ACTION_EDIT;
	if (label === "Write") return FOLD_ACTION_WRITE;
	if (label === "Patch") return FOLD_ACTION_PATCH;
	if (label === "Ask") return FOLD_ACTION_ASK;
	if (label === "Agent") return FOLD_ACTION_AGENT;
	if (label === "Task") return FOLD_ACTION_TASK;
	if (label === "Skill") return FOLD_ACTION_SKILL;
	if (label === "MCP") return FOLD_ACTION_MCP;
	return WORKED_LINE_FG;
}

function foldRunActionEntries(run: FoldRun): Array<{ label: string; count: number }> {
	const counts = new Map<string, number>();
	if (run.thoughts.length > 0) counts.set("Think", run.thoughts.length);
	for (const tool of run.tools) {
		const label = foldActionLabel(getToolName(tool));
		counts.set(label, (counts.get(label) ?? 0) + 1);
	}
	return [...counts.entries()].map(([label, count]) => ({ label, count }));
}

function styledFoldActionCount(label: string, count: number): string {
	const plain = formatFoldActionCount(label, count);
	const suffix = plain.slice(label.length + 1);
	const countText = suffix.replace(/^×\s*/u, "");
	return `${foldActionColor(label)}${label}${TRANSPARENT_RESET} ${FG_DIM}×${TRANSPARENT_RESET} ${countText}`;
}

/**
 * 动作计数片段（不含行首缩进），供叙述行拼在同一行。
 * budget 是这一行剩余可用宽度；为 0 时返回空串，避免只剩一个 `+N` 标记。
 */
function foldRunActionText(run: FoldRun, budget: number): string {
	if (budget <= 0) return "";
	const entries = foldRunActionEntries(run);
	if (entries.length === 1 && entries[0].label === "Think" && entries[0].count === 1) return "";
	const segments = entries.map((entry) => styledFoldActionCount(entry.label, entry.count));
	const fit = fitFoldActionPrefix(entries.map((entry) => entry.count), budget, MAX_FOLD_ACTION_SEGMENTS, (segmentCount, hiddenCount) => {
		const text = segments.slice(0, segmentCount).join("   ");
		return visibleWidth(hiddenCount > 0 ? `${text}   +${hiddenCount}` : text);
	});
	const shown = segments.slice(0, fit.segments);
	if (fit.hidden > 0 && shown.length > 0) shown.push(`${FG_DIM}+${TRANSPARENT_RESET} ${fit.hidden}`);
	return shown.join("   ");
}

function isFoldThoughtActive(thought: any): boolean {
	return isLiveThinkingMessage(thought, thought?.lastMessage) || thought?.isStreaming === true;
}

function foldRunPendingTools(run: FoldRun): any[] {
	return run.tools.filter((tool) => getToolStatusForGroup(tool) === "pending");
}

function foldRunIsActive(run: FoldRun): boolean {
	return run.thoughts.some(isFoldThoughtActive) || foldRunPendingTools(run).length > 0;
}

function foldRunElapsed(run: FoldRun, now: number): number | undefined {
	const toolElapsed = foldElapsedMs(run.tools.map((tool) => {
		const state = (tool as any)?.rendererState;
		return { startedAt: state?.[BASH_STARTED_AT_KEY], endedAt: state?.[BASH_ENDED_AT_KEY] };
	}), now);
	if (toolElapsed !== undefined) return toolElapsed;
	if (foldRunIsActive(run) && typeof currentAgentWorkStartMs === "number") {
		return Math.max(0, now - currentAgentWorkStartMs);
	}
	const thoughtElapsed = run.thoughts.reduce((total, thought) => total + getMessageThinkingDurationMs(thought?.lastMessage), 0);
	return thoughtElapsed > 0 ? thoughtElapsed : undefined;
}

function foldRunStatus(run: FoldRun): ToolStatus {
	if (run.tools.some((tool) => getToolStatusForGroup(tool) === "error")) return "error";
	if (foldRunIsActive(run)) return "pending";
	return "success";
}

function foldThinkingTotalMs(run: FoldRun): number {
	let total = 0;
	for (const thought of run.thoughts) total += getMessageThinkingDurationMs(thought?.lastMessage);
	return total;
}

/** 第一行叙述句：思考写总耗时，其余动作写次数。 */
function foldRunNarrative(run: FoldRun, active: boolean): string {
	const entries = foldRunActionEntries(run);
	const duration = formatThoughtDuration(foldThinkingTotalMs(run));
	const phrases = entries.map((entry) => active
		? foldActionProgressivePhrase(entry.label, entry.count)
		: foldActionPastPhrase(entry.label, entry.count, duration));
	const [first, ...rest] = phrases;
	return [first.charAt(0).toUpperCase() + first.slice(1), ...rest].join(", ");
}

function foldRunHeader(run: FoldRun, width: number): string {
	// 运行标记只看当前是否还有活动：同一条记录里先前有命令失败，不应让
	// 仍在运行的工作退回成状态点，也不应改成过去式的叙述。
	const marker = foldRunMarkerState(foldRunIsActive(run), foldRunStatus(run));
	const glyph = marker === "running"
		? `${FOLD_WORKING_FG}${foldWorkingGlyph(_foldWorkingFrame)}${TRANSPARENT_RESET}`
		: groupStatusLight(marker);
	const elapsed = foldRunElapsed(run, Date.now());
	const elapsedText = elapsed === undefined ? "" : formatBashDuration(elapsed);
	const head = ` ${glyph} ${foldRunNarrative(run, marker === "running")}`;
	// 计数与叙述同行，耗时仍靠右；宽度不足时先舍弃计数，再交给截断处理。
	const reserved = visibleWidth(head) + (elapsedText ? visibleWidth(elapsedText) + 4 : 0);
	const counts = foldRunActionText(run, width - reserved - 3);
	const left = counts ? `${head}   ${counts}` : head;
	if (!elapsedText) return clampLineWidth(left, width);
	return alignTrailingMarkedLine(`${left}${TRAILING_MARK}${themeFoldMuted(elapsedText)}`, width);
}

function cleanFoldPreviewLines(raw: string): string[] {
	return raw
		.replaceAll(WRAP_MARK, "")
		.replaceAll(CLIP_MARK, "")
		.split("\n")
		.map((line) => stripAnsi(line).replace(/^[└├│─━╭╮╰╯┌┐\s]+/u, "").trimEnd())
		.filter((line) => line.trim().length > 0)
		.filter((line) => !foldHintFragments().some((hint) => line.includes(hint)));
}

function foldToolLivePreview(tool: any, limit: number): string[] {
	if (limit <= 0) return [];
	const name = getToolName(tool);
	// 变更类工具的调用渲染器里带着 diff，优先取它；其余工具只取结果渲染器，
	// 否则拿到的只是调用行本身，和前面的动作标签行完全重复。
	const preferCall = name === "edit" || name === "write" || name === "apply_patch";
	const primary = preferCall ? tool?.callRendererComponent : tool?.resultRendererComponent;
	const secondary = preferCall ? tool?.resultRendererComponent : undefined;
	let raw = typeof primary?.value === "string" ? primary.value : "";
	let fromCall = preferCall && raw.length > 0;
	if (!raw && typeof secondary?.value === "string") {
		raw = secondary.value;
		fromCall = !preferCall;
	}
	let lines = cleanFoldPreviewLines(raw);
	if (fromCall && lines.length > 1) lines = lines.slice(1);
	if (name === "bash" && lines.length > limit) lines = lines.slice(-limit);
	return lines.slice(0, limit);
}

function styleFoldPreviewText(line: string): string {
	const trimmed = line.trimStart();
	if (trimmed.startsWith("+") && !trimmed.startsWith("+++")) return `${FG_ADD}${line}${TRANSPARENT_RESET}`;
	if (trimmed.startsWith("-") && !trimmed.startsWith("---")) return `${FG_DEL}${line}${TRANSPARENT_RESET}`;
	return themeFoldMuted(line);
}

function foldThinkingPreview(run: FoldRun): string {
	for (let i = run.thoughts.length - 1; i >= 0; i--) {
		const message = run.thoughts[i]?.lastMessage;
		const blocks = Array.isArray(message?.content) ? message.content : [];
		for (let j = blocks.length - 1; j >= 0; j--) {
			const text = blocks[j]?.type === "thinking" && typeof blocks[j]?.thinking === "string" ? blocks[j].thinking : "";
...
/**
 * 运行中的实时行。区块高度在运行期间保持不变，会话内容才不会上下跳动，
 * 因此这里只会输出一行：思考中显示思考末尾，其余显示最新一个工具的最新内容。
 */
function formatFoldLiveLines(run: FoldRun, width: number): string[] {
	const branch = `${FG_DIM}└${TRANSPARENT_RESET}`;
	const pending = foldRunPendingTools(run);
	if (pending.length === 0) {
		if (!run.thoughts.some(isFoldThoughtActive)) return [];
		const label = `${foldActionColor("Think")}Think${TRANSPARENT_RESET}`;
		return [clampLineWidth(`   ${branch} ${label}  ${themeFoldMuted(foldThinkingPreview(run))}`, width)];
	}
	const tool = pending[pending.length - 1];
	const label = foldActionLabel(getToolName(tool));
	const preview = liveToolPreviewEnabled() ? foldToolLivePreview(tool, 1)[0] : undefined;
	const detail = preview
		? `  ${styleFoldPreviewText(preview)}`
		: (() => {
			const summary = getToolArgSummary(tool);
			return summary ? `  ${summary}` : "";
		})();
	return [clampLineWidth(`   ${branch} ${foldActionColor(label)}${label}${TRANSPARENT_RESET}${detail}`, width)];
}

function foldMutationSummary(tools: any[]): string[] {
	...
	for (const tool of tools) {
		...
		const path = getStringArg(tool?.args ?? {}, "path", "file_path");
		if (path) files.add(path);
		const details = tool?.result?.details;
		if (details?._type === "editInfo") {
			added += details.added ?? 0;
			removed += details.removed ?? 0;
		} else if (details?._type === "multiEditInfo") {
			added += details.totalAdded ?? 0;
			removed += details.totalRemoved ?? 0;
		} else if (details?._type === "diff") {
			added += details.diff?.added ?? 0;
			removed += details.diff?.removed ?? 0;
		} else if (details?._type === "new") {
			added += details.lines ?? 0;
		}
	}
	const parts: string[] = [];
	if (files.size > 0) parts.push(`${files.size} file${files.size === 1 ? "" : "s"} changed`);
	if (added > 0) parts.push(`${FG_ADD}+${added}${TRANSPARENT_RESET}`);
	if (removed > 0) parts.push(`${FG_DEL}−${removed}${TRANSPARENT_RESET}`);
	return parts;
}

function foldRunErrorText(run: FoldRun, width: number): string {
	const failed = run.tools.find((tool) => getToolStatusForGroup(tool) === "error");
	if (!failed) return "";
	return foldToolResultText(failed, Math.max(1, width - 8));
}

/** 第三行：成功与失败计数，附文件修改统计与展开入口。 */
function foldCompletionLines(run: FoldRun, width: number): string[] {
	if (run.tools.length === 0) return [];
	const counts = countToolStatuses(run.tools);
	const parts: string[] = [];
	if (counts.success > 0) parts.push(`${TOOL_STATUS_SUCCESS}${counts.success} done${TRANSPARENT_RESET}`);
	if (counts.error > 0) parts.push(`${TOOL_STATUS_ERROR}${counts.error} failed${TRANSPARENT_RESET}`);
	parts.push(...foldMutationSummary(run.tools));
	const errorText = foldRunErrorText(run, width);
	const branch = errorText ? "├" : "└";
	const separator = ` ${FG_DIM}·${TRANSPARENT_RESET} `;
	const hint = width >= 80 ? `   ${configuredKeyHint("app.tools.expand", "ctrl+o", "for details")}` : "";
	const summary = clampLineWidth(`   ${FG_DIM}${branch}${TRANSPARENT_RESET} ${parts.join(separator)}${hint}`, width);
	if (!errorText) return [summary];
	return [summary, clampLineWidth(`   ${FG_DIM}└${TRANSPARENT_RESET} ${TOOL_STATUS_ERROR}${errorText}${TRANSPARENT_RESET}`, width)];
}

function formatFoldRunLines(run: FoldRun, width: number): string[] {
	const safeWidth = Number.isFinite(width) ? Math.max(1, Math.floor(width)) : 1;
	const lines = [foldRunHeader(run, safeWidth)];
	if (foldRunIsActive(run)) lines.push(...formatFoldLiveLines(run, safeWidth));
	else lines.push(...foldCompletionLines(run, safeWidth));
	return ["", ...lines];
}

function nearestFoldSibling(children: any[], index: number, step: 1 | -1): any {
	for (let i = index + step; i >= 0 && i < children.length; i += step) {
		if (isIgnorableToolSeparator(children[i])) continue;
		return children[i];
	}
	return undefined;
}

/** 批次内部的空行：左右两侧都是批次成员时不再占一行。 */
function isFoldRunInteriorSpacer(spacer: any): boolean {
	const children = spacer?.[COMPONENT_PARENT]?.children;
	if (!Array.isArray(children)) return false;
	const index = children.indexOf(spacer);
	if (index === -1) return false;
	return isFoldRunParticipant(nearestFoldSibling(children, index, -1))
		&& isFoldRunParticipant(nearestFoldSibling(children, index, 1));
}
...
class ToolGroupComponent extends Container {
	...
	addTool(tool: any): void {
		...
		tool.setExpanded?.(isToolFoldExpanded(toolFoldLevel));
	...
	setExpanded(expanded: boolean): void {
		// Pi 的布尔值只用来识别 ctrl+o 按键，实际展开状态由当前档位决定。
		notePiToolsExpanded(expanded);
		this.applyFoldExpansion();
	}

	/** 按当前档位统一设置本组与子工具的展开状态；不参与按键计数。 */
	applyFoldExpansion(): void {
		const expanded = isToolFoldExpanded(toolFoldLevel);
	...
	render(width: number): string[] {
		...
		const safeWidth = Number.isFinite(width) ? Math.max(1, Math.floor(width)) : 1;
		if (toolFoldLevel === 2) {
			const run = getFoldRun(this);
			if (!run) return [];
			if (run.first !== this) return [];
			return formatFoldRunLines(run, safeWidth);
		}
...
function patchUserMessageRender(): void {
	...
	proto.render = function patchedUserMessageRender(width: number) {
		...
		// Pi 自带的填充底色直接保留，不再套外框。
		const lines = originalRender.call(this, width);
		if (!Array.isArray(lines) || lines.length === 0) return lines;
		return storeMessageRenderCache(this, width, applyTerminalCopyZones(lines.map(stripOsc133Zones)));
	};
...
function patchAssistantMessages(): void {
	...
	if (typeof originalRender === "function" && !proto[ASSISTANT_RENDER_PATCH_FLAG]) {
		proto.render = function patchedAssistantMessageRender(width: number) {
			if (toolFoldLevel === 2 && isFoldThoughtParticipant(this)) {
				const run = getFoldRun(this);
				if (run) {
					if (run.first !== this) return [];
					return formatFoldRunLines(run, width);
				}
			}
			const cached = messageRenderCacheHit(this, width);
			if (cached) return cached;
			...
			const lines = originalRender.call(this, width);
			if (!Array.isArray(lines) || lines.length === 0) return lines;
			if (toolFoldLevel === 2 && assistantMessageHasThinking(this) && !foldThoughtClaimedByPreviousRun(this)) {
				const run: FoldRun = { first: this, tools: [], thoughts: [this] };
				return [...formatFoldRunLines(run, width), ...lines];
			}
...
/**
 * 把三档层级接到工具行与空行上：工具行的展开状态跟随档位，
 * 整组压缩时只有批次里第一个工具行负责渲染，其余工具行与内部空行让位。
 */
function patchToolFoldLevels(): void {
	const toolProto = ToolExecutionComponent.prototype as any;
	if (!toolProto[TOOL_FOLD_PATCH_FLAG]) {
		const originalSetExpanded = toolProto.setExpanded;
		toolProto.setExpanded = function patchedFoldAwareSetExpanded(expanded: boolean): void {
			notePiToolsExpanded(expanded);
			originalSetExpanded.call(this, foldPropagationDepth > 0 ? expanded : isToolFoldExpanded(toolFoldLevel));
		};
		const originalToolRender = toolProto.render;
		toolProto.render = function patchedFoldAwareToolRender(width: number): string[] {
			if (toolFoldLevel === 2) {
				const run = getFoldRun(this);
				if (run) {
					if (run.first !== this) return [];
		...
		toolProto[TOOL_FOLD_PATCH_FLAG] = true;
	}

	const spacerProto = Spacer.prototype as any;
	if (!spacerProto[SPACER_FOLD_PATCH_FLAG]) {
		const originalSpacerRender = spacerProto.render;
		spacerProto.render = function patchedFoldAwareSpacerRender(width: number): string[] {
			if (toolFoldLevel === 2 && isFoldRunInteriorSpacer(this)) return [];
			return originalSpacerRender.call(this, width);
		};
		spacerProto[SPACER_FOLD_PATCH_FLAG] = true;
	}
}
...
/** 运行中的压缩批次使用的 asterisk 帧下标，与闪烁计时器同步推进。 */
let _foldWorkingFrame = 0;
...
function themePaletteFingerprint(theme: any): string {
	const keys = [
		"success", "error", "warning", "borderMuted", "borderAccent", "accent", "muted", "dim",
		"thinkingText", "thinkingLow", "thinkingMedium", "thinkingHigh", "bashMode",
		"mdLink", "mdHeading", "mdCode", "mdListBullet",
		"syntaxFunction", "syntaxKeyword", "syntaxString", "syntaxType",
...
const _claudeStyleDefaults = {
	...
	FOLD_WORKING_FG: "\x1b[38;5;208m",
	FOLD_ACTION_THINK: "\x1b[35m",
	FOLD_ACTION_BASH: "\x1b[33m",
	FOLD_ACTION_READ: "\x1b[34m",
	FOLD_ACTION_SEARCH: "\x1b[36m",
	FOLD_ACTION_EDIT: "\x1b[32m",
	FOLD_ACTION_WRITE: "\x1b[38;5;37m",
	FOLD_ACTION_PATCH: "\x1b[38;5;70m",
	FOLD_ACTION_ASK: "\x1b[38;5;171m",
	FOLD_ACTION_AGENT: "\x1b[38;5;209m",
	FOLD_ACTION_TASK: "\x1b[38;5;69m",
	FOLD_ACTION_SKILL: "\x1b[38;5;214m",
	FOLD_ACTION_MCP: "\x1b[38;5;99m",
	FOLD_ACTION_LIST: "\x1b[38;5;45m",
};
...
function resetThemePalette(): void {
	...
	FOLD_WORKING_FG = _claudeStyleDefaults.FOLD_WORKING_FG;
	FOLD_ACTION_THINK = _claudeStyleDefaults.FOLD_ACTION_THINK;
	FOLD_ACTION_BASH = _claudeStyleDefaults.FOLD_ACTION_BASH;
	FOLD_ACTION_READ = _claudeStyleDefaults.FOLD_ACTION_READ;
	FOLD_ACTION_SEARCH = _claudeStyleDefaults.FOLD_ACTION_SEARCH;
	FOLD_ACTION_EDIT = _claudeStyleDefaults.FOLD_ACTION_EDIT;
	FOLD_ACTION_WRITE = _claudeStyleDefaults.FOLD_ACTION_WRITE;
	FOLD_ACTION_PATCH = _claudeStyleDefaults.FOLD_ACTION_PATCH;
	FOLD_ACTION_ASK = _claudeStyleDefaults.FOLD_ACTION_ASK;
	FOLD_ACTION_AGENT = _claudeStyleDefaults.FOLD_ACTION_AGENT;
	FOLD_ACTION_TASK = _claudeStyleDefaults.FOLD_ACTION_TASK;
	FOLD_ACTION_SKILL = _claudeStyleDefaults.FOLD_ACTION_SKILL;
	FOLD_ACTION_MCP = _claudeStyleDefaults.FOLD_ACTION_MCP;
	FOLD_ACTION_LIST = _claudeStyleDefaults.FOLD_ACTION_LIST;
...
type FoldActionColorKey =
	| "FOLD_ACTION_THINK" | "FOLD_ACTION_BASH" | "FOLD_ACTION_LIST" | "FOLD_ACTION_READ"
	| "FOLD_ACTION_SEARCH" | "FOLD_ACTION_EDIT" | "FOLD_ACTION_WRITE" | "FOLD_ACTION_PATCH"
	| "FOLD_ACTION_ASK" | "FOLD_ACTION_AGENT" | "FOLD_ACTION_TASK" | "FOLD_ACTION_SKILL"
	| "FOLD_ACTION_MCP";

/** 主题色冲突时的备用色，按区分度排序，保证每个动作最终拿到不同颜色。 */
const FOLD_ACTION_SPARE_COLORS = [
	"\x1b[35m", "\x1b[33m", "\x1b[34m", "\x1b[36m", "\x1b[32m",
	"\x1b[38;5;45m", "\x1b[38;5;209m", "\x1b[38;5;171m", "\x1b[38;5;69m",
	"\x1b[38;5;214m", "\x1b[38;5;99m", "\x1b[38;5;37m", "\x1b[38;5;70m",
] as const;

/**
 * 动作颜色：优先使用主题里语义对应的颜色键。主题常常把多个键映射到
 * 同一个值时，重复的动作退回自己的内置色，再重复则取一个尚未使用的备用色，
 * 使同一行里的动作始终可以靠颜色区分。
 */
function resolveFoldActionColors(theme: any): Record<string, string> {
	const candidates: Array<[string, string | null, FoldActionColorKey]> = [
		["Think", safeFgAnsi(theme, "thinkingMedium") ?? safeFgAnsi(theme, "thinkingText"), "FOLD_ACTION_THINK"],
		["Bash", safeFgAnsi(theme, "warning") ?? safeFgAnsi(theme, "bashMode"), "FOLD_ACTION_BASH"],
		["List", safeFgAnsi(theme, "mdListBullet") ?? safeFgAnsi(theme, "syntaxString"), "FOLD_ACTION_LIST"],
		["Read", safeFgAnsi(theme, "mdLink") ?? safeFgAnsi(theme, "accent"), "FOLD_ACTION_READ"],
		["Search", safeFgAnsi(theme, "syntaxFunction") ?? safeFgAnsi(theme, "accent"), "FOLD_ACTION_SEARCH"],
		["Edit", safeFgAnsi(theme, "success"), "FOLD_ACTION_EDIT"],
		["Write", safeFgAnsi(theme, "syntaxType") ?? safeFgAnsi(theme, "mdLink"), "FOLD_ACTION_WRITE"],
		["Patch", safeFgAnsi(theme, "toolDiffAdded") ?? safeFgAnsi(theme, "success"), "FOLD_ACTION_PATCH"],
		["Ask", safeFgAnsi(theme, "mdHeading") ?? safeFgAnsi(theme, "syntaxKeyword"), "FOLD_ACTION_ASK"],
		["Agent", safeFgAnsi(theme, "syntaxKeyword") ?? safeFgAnsi(theme, "accent"), "FOLD_ACTION_AGENT"],
		["Task", safeFgAnsi(theme, "accent") ?? safeFgAnsi(theme, "syntaxFunction"), "FOLD_ACTION_TASK"],
		["Skill", safeFgAnsi(theme, "mdCode") ?? safeFgAnsi(theme, "syntaxString"), "FOLD_ACTION_SKILL"],
		["MCP", safeFgAnsi(theme, "thinkingHigh") ?? safeFgAnsi(theme, "thinkingLow"), "FOLD_ACTION_MCP"],
	];
	const resolved: Record<string, string> = {};
	const used = new Set<string>();
	for (const [label, candidate, defaultKey] of candidates) {
		const fallback = _claudeStyleDefaults[defaultKey];
		const spare = FOLD_ACTION_SPARE_COLORS.find((color) => !used.has(color));
		const value = candidate && !used.has(candidate)
			? candidate
			: (!used.has(fallback) ? fallback : (spare ?? fallback));
		used.add(value);
...
function applyThemePaletteIfNeeded(theme: any): void {
	...
	FOLD_WORKING_FG = safeFgAnsi(theme, "borderAccent") ?? safeFgAnsi(theme, "accent") ?? FOLD_WORKING_FG;
	const foldActionColors = resolveFoldActionColors(theme);
	FOLD_ACTION_THINK = foldActionColors.Think;
	FOLD_ACTION_BASH = foldActionColors.Bash;
	FOLD_ACTION_LIST = foldActionColors.List;
	FOLD_ACTION_READ = foldActionColors.Read;
	FOLD_ACTION_SEARCH = foldActionColors.Search;
	FOLD_ACTION_EDIT = foldActionColors.Edit;
	FOLD_ACTION_WRITE = foldActionColors.Write;
	FOLD_ACTION_PATCH = foldActionColors.Patch;
	FOLD_ACTION_ASK = foldActionColors.Ask;
	FOLD_ACTION_AGENT = foldActionColors.Agent;
	FOLD_ACTION_TASK = foldActionColors.Task;
	FOLD_ACTION_SKILL = foldActionColors.Skill;
	FOLD_ACTION_MCP = foldActionColors.MCP;
...
export default function (pi: ExtensionAPI) {
	...
	const notifyToolStatus = (ctx: any): void => {
		...
		ctx.ui.notify([
			...
			`Detail level: ${toolFoldLevelName(toolFoldLevel)} (default ${toolFoldLevelName(configuredToolFoldLevel())}; ${rawKeyHint("ctrl+o", "cycles")})`,
	...
	pi.registerCommand("cc-tools", {
		description: "Control tool UI: style, grouped rows, and the Ctrl+O detail levels",
	...
	pi.on("session_start", async (_event, ctx) => {
		_clearAllBlinkContexts();
		resetToolFoldLevel(ctx?.hasUI ? ctx.ui : undefined);
	});
