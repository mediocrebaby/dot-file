...
function searchProviderSchema(description: string) {
	return StringEnum([...SEARCH_PROVIDERS], { description });
}
...
function toCuratorSearchEntries(response: AttributedSearchResponse): CuratorSearchEntry[] {
	const entries: CuratorSearchEntry[] = [{
		answer: response.answer,
		results: response.results.map(source => ({ ...source, domain: extractDomain(source.url) })),
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("websearch", {
		...
		handler: async (args, ctx) => {
			...
			const curatorTimeoutSeconds = bootstrap.timeoutSeconds;
			const currentSearchProvider: SearchProviderSelection = initialProvider;
			const summaryContext: SummaryGenerationContext = {
				model: ctx.model,
				modelRegistry: ctx.modelRegistry,
				cwd: ctx.cwd,
