...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, and **`/committing-changes`** commits the result. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **`/docx`**: create, read, and edit Word documents (`.docx`, `.dotx`). Reach for it whenever a Word file is involved: writing a report, memo, letter, or template; extracting or rearranging content from an existing document; redlining with tracked changes, comments, or images. Word files only — no PDFs or spreadsheets.
- **`/committing-changes`**: commit uncommitted work in a Git repository. Reach for it any time you want changes committed. It splits the diff into logical topics — one commit per topic, `git add -p` for files that mix several — and defaults to Chinese commit messages unless you specify otherwise.
- **`/baibai-aigc`**: de-AIGC (降AIGC) rewriting for Chinese and English technical/academic text: strips AI tone while preserving facts, terminology, and structure. Reach for it on 降AIGC, 论文去AI味, or humanising a paper or technical document. One round per invocation: Chinese text runs two ordered rounds (`baibaiaigc1` → `baibaiaigc2`), English one, chained across conversations through the 降AIGC记录 — so continuing to the next round means opening a fresh conversation and invoking again.
- **`/blackbox-whitebox-reversing`**: the reverse-engineering loop for native targets: black-box observation collects an oracle of input→output pairs and narrows the analysis to a few functions, white-box decompilation recovers the exact algorithm, and the reimplementation is verified against the oracle byte-for-byte. Reach for it when you want to recover or reimplement closed logic: an opaque "custom crypto" routine, a protocol check, a CTF challenge. It opens with an authorisation gate — settle that the target is yours, a CTF, an authorised pentest, or study material before any target runs.
