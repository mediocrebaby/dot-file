...
仅在主入口已选择 HTML 交付时使用本参考；普通审查仍默认对话 Markdown。HTML 默认输出到系统临时目录，单文件、离线可读；样式和 SVG 内嵌，不依赖 CDN。只有用户接受联网依赖时才选择 Mermaid/Tailwind CDN，并在报告中说明。语法术语参考 [design.md](references/design.md)，领域名称优先服从项目 CONTEXT。
...
- 变化前后的关键差别：可用文字说明；图确实有助理解时，再选调用链、状态流、层次或接口宽度图，不要求每个候选配图。
